...
apply_blur_filter( animated_gif * image, int size, int threshold )
{
    ...
    // Since the task (copying pixel values) is uniform and doesn't vary in complexity, static, 1 scheduling allows threads to work without frequent synchronization overhead or task assignment. use cache locality
	...
	for(j=0; j<height-1; j++)
	{
		for(k=0; k<width-1; k++)
		{
            ...
            #pragma omp parallel 
            {
                ...
                //don t wait because they are independent
...
apply_sobel_filter( animated_gif * image )
{
    ...
    for ( i = 0 ; i < image->n_images ; i++ )
    {
        ...
        #pragma omp parallel for collapse(1) private(j,k)
        for(j=1; j<height-1; j++)
        {
            for(k=1; k<width-1; k++)
            {
...
main( int argc, char ** argv )
{
    ...
    gettimeofday(&t2, NULL);
    duration = (t2.tv_sec -t1.tv_sec)+((t2.tv_usec-t1.tv_usec)/1e6);
    printf("Sobel filter took %lf s\n",duration ) ;

    /* FILTER Timer stop */
    gettimeofday(&t3, NULL);

    
    duration = (t3.tv_sec -t0.tv_sec)+((t3.tv_usec-t0.tv_usec)/1e6);
